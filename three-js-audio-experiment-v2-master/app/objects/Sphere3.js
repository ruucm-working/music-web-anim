...
export default class Sphere3 extends BaseThreeObj {
  constructor() {
    ...
    //   alphamap: alphaMap
    // });
    this.mat = new THREE.MeshBasicMaterial({ color: "#45844B", transparent: true, side: THREE.DoubleSide, alphaTest: 0.5, opacity: 1, roughness: 1 });
  ...
  update(audioData) {
    for (let i = 0; i < this.mesh.geometry.vertices.length; i++) {
      ...
      this.mesh.geometry.vertices[i].x = this.initialGeomVertices[i].x * (audioData[i] / 120) * -1;
      this.mesh.geometry.vertices[i].y = this.initialGeomVertices[i].y * (audioData[i] / 320) * -1;
      this.mesh.geometry.vertices[i].z = this.initialGeomVertices[i].z * (audioData[i] / 120) * -1;
    }
